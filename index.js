...
let setting = {
  dir:'test-ApiFile',
  swaggerUrl:'http://172.16.8.102:8529/salary/v2/api-docs',
...
http.get(setting.swaggerUrl,setting.options,(res) => {
...
function paramsFn(parameters,definitions){
  ...
  parameters.forEach((param) => {
    ...
    if(Reflect.has(param,'schema')){
          ...
          if(type && type === 'object'){
            if(properties) {
              for(prop in properties) {
                if(required && Array.isArray(required) && required.includes(prop)){
                  paramsStr += backTemplate(prop,true,properties[prop].type,properties[prop].description)
                }else {
                  paramsStr += backTemplate(prop,false,properties[prop].type,properties[prop].description)
                }
              }
            }
          }
      ...
      if(param.required){
        paramsStr += backTemplate(param.name,true,param.type,param.description)
      }else {
        paramsStr += paramsStr += backTemplate(param.name,true,param.type,param.description)
      }
    }
  })
...
function generateApiFile(apiDoc){
  ...
  for(let url in paths) {
    for(let method in paths[url]) {
      ...
      let apiUrl;
      if(parametersIn.includes('path')) { // 接口参数在请求路径上
...
function saveApiFile(tagObj){
  ...
  for(prop in tagObj) {
    fs.writeFileSync('./'+setting.dir+'/'+tagObj[prop].name+'.js',tagObj[prop].apiStr,{
        encoding:'utf-8'
    })
  }
}
...
// paths[url][method]  .summary 接口注释信息 url.split('/')[leng-1] 得到函数名字 
